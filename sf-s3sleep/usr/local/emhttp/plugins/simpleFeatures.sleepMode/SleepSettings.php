...
<?
$plugin = 's3.sleep';
$ini = parse_ini_file("$unraid/simpleFeatures.$plugin.ini");
$sName = "s3_sleep";
...
<?endfor;?>
  ...
  <option value='' selected></option>
<?for ($h=0; $h<24; $h++):?>
<?=mk_option_check($ini['stopHour'], sprintf("%02d", $h), sprintf("%02d:00 - %02d:59", $h, $h))?>
<?endfor;?>
  </select></td>
  </tr>
...
<?=mk_option($ini['checkTCP'], "-n", "Yes")?>
  ...
  <td><input type="text" name="idle" maxlength="4"></td>
  </tr>
  <tr><td>Wait for device inactivity (address):</td>
  <td><input type="text" name="pingIP" maxlength="200" value="<?=$ini['pingIP']?>"></td>
  </tr>
...
<?=mk_option($ini['dhcpRenew'], "-R", "Yes")?>
  ...
  <td><input type="text" name="setWol" maxlength="8" value="<?=$ini['setWol']?>"> <?if ($wakeon):?>Wake-on: <strong><?=$wakeon[1]?></strong> (available options: <strong><?=$wakeon[0]?></strong>)<?endif;?></td>
  </tr>
